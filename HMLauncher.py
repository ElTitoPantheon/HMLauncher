...
class GameBananaWidget(QWidget):
    ...
    def card(self, m):
        ...
        c.setStyleSheet("background:#282828;border:1px solid #3a3a3a;")
        ...
        info = QFrame()
        info.setFixedHeight(CARD_H - IMG_H)
        info.setStyleSheet("background:#3a3a3a;")
        ...
        author = QLabel(m["author"])
        author.setStyleSheet("color:#bbb;font-size:9px;")

        stats = QLabel(f"❤ {m['likes']}   💬 {m['posts']}   👁 {m['views']}")
        ...
        btn.clicked.connect(
              lambda _, mod_id=m["id"], mod_type=m["type"]:
              self.launcher.abrir_detalles_mod(mod_id, mod_type)
          )
        if m["thumb"]:
            if m["thumb"] in self.img_cache:
                img.setPixmap(self.img_cache[m["thumb"]])
            else:
